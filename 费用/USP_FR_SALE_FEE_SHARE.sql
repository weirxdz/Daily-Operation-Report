CREATE PROCEDURE [dbo].[USP_FR_SALE_FEE_SHARE]
@IYEAR INT=2021,
@IPERIOD INT=7
AS
BEGIN
	...
	BEGIN--********变量定义开始********
		...
		DECLARE @NUM INT--中间表行数
		DECLARE @NSTEP INT--中间表行数计数
		DECLARE @I  INT--循环月份数
		DECLARE @IIYEAR INT
		DECLARE @IIPERIOD INT
		DECLARE @IDEPT VARCHAR(100)
		DECLARE @ICUST VARCHAR(100)
		DECLARE @IFEE DECIMAL(20,2)

		DECLARE @INMNY_Y DECIMAL(20,2)
		DECLARE @NMNY_D_M DECIMAL(20,2)
		DECLARE @NMNY_D_Y DECIMAL(20,2)
		DECLARE @NMNY_Z_M DECIMAL(20,2)
		DECLARE @NMNY_Z_Y DECIMAL(20,2)
		DECLARE @IFEE_Z DECIMAL(20,2)
	...
	BEGIN--********临时表定义开始********
		...
		CREATE TABLE #FR_FEEDETAIL_0
			(
				I_ID INT,IYEAR INT ,IPERIOD INT ,PZDEPT VARCHAR(200) ,PZDEPNAME VARCHAR(200) ,PZCUST VARCHAR(100),PZCUSTNAME VARCHAR(100),CUSTDEPT VARCHAR(100),CUSTDEPTNAME VARCHAR(100),DIM_DEPT VARCHAR(200) ,DIM_DEPTNAME VARCHAR(200) ,DIM_CUST VARCHAR(100),DIM_CUSTNAME VARCHAR(100) ,FEE DECIMAL(20,2)
			)
		...
		*/
		CREATE TABLE #FR_SALE_DETAILS
			(
				AutoID VARCHAR(100),IYEAR INT ,IPERIOD INT ,FPDEPT VARCHAR(100),FPDEPTNAME VARCHAR(100),FPCUST VARCHAR(100),FPCUSTNAME VARCHAR(100),CUSTDEPT VARCHAR(100),CUSTDEPTNAME VARCHAR(100),DIM_DEPT VARCHAR(100),DIM_DEPTNAME VARCHAR(100) ,DIM_CUST VARCHAR(100),DIM_CUSTNAME VARCHAR(100) , NMNY DECIMAL(20,2)
			)
		/*能够同时匹配部门和客户的数据，数据是汇总数据*/
	...
	BEGIN/*组建维度表开始*/
		IF(@IPERIOD = 12)
			BEGIN
				...
				FROM (
					SELECT DISTINCT @IYEAR AS IYEAR,@IPERIOD AS IPERIOD,ISNULL(CUST.CCUSDEPART,'未匹配') AS DIM_DEPT,D.CDEPNAME AS DIM_DEPTNAME,ISNULL(H.CCUSCODE,'未匹配') AS DIM_CUST,CASE WHEN H.CCUSCODE IS NOT NULL THEN CUST.CCUSNAME ELSE '未匹配'END AS DIM_CUSTNAME
					...
					UNION
					SELECT DISTINCT @IYEAR-1 AS IYEAR,@IPERIOD AS IPERIOD,ISNULL(CUST.CCUSDEPART,'未匹配') AS DIM_DEPT,D.CDEPNAME AS DIM_DEPTNAME,ISNULL(H.CCUSCODE,'未匹配') AS DIM_CUST,CASE WHEN H.CCUSCODE IS NOT NULL THEN CUST.CCUSNAME ELSE '未匹配'END AS DIM_CUSTNAME
					FROM SALEBILLVOUCHS B
					LEFT JOIN SALEBILLVOUCH H ON B.SBVID = H.SBVID
					LEFT JOIN CUSTOMER CUST ON H.CCUSCODE = CUST.CCUSCODE
					LEFT JOIN DEPARTMENT D ON CUST.CCUSDEPART = D.CDEPCODE
	...
	BEGIN/*取费用明细数据开始*/
		...
		SELECT A.I_ID,A.IYEAR,A.IPERIOD,A.PZDEPT,A.PZDEPTNAME,A.PZCUST,A.PZCUSTNAME,A.CUSTDEPT,A.CUSTDEPTNAME,A.DIM_DEPT,A.DIM_DEPTNAME,A.DIM_CUST,A.DIM_CUSTNAME,A.FEE
		FROM
	...
	BEGIN/*取收入明细数据开始*/
		IF(@IPERIOD = 12)
			BEGIN
				...
				SELECT T.AutoID,T.IYEAR,T.IPERIOD AS IPERIOD,T.FPDEPT,T.FPDEPTNAME,T.FPCUST,T.FPCUSTNAME,T.CUSTDEPT,T.CUSTDEPTNAME,T.DIM_DEPT,T.DIM_DEPTNAME,T.DIM_CUST,T.DIM_CUSTNAME,NTAXMNY AS NMNY
				FROM (
	...
	BEGIN/*搭建分析表框架，把维度表写入 #FR_FEEDETAIL_1 */
		INSERT INTO #FR_FEEDETAIL_1
			...
			FROM #DIM
	END/*搭建分析表框架，把维度表写入 #FR_FEEDETAIL_1 结束*/
	BEGIN--********月度费用、收入数据写入开始********
		/*更新月度数据_部门客户都匹配的数据*/
			--费用数据
	...
	END--********月度费用、收入数据写入结束********
	BEGIN--查询结果开始
		SELECT T.IYEAR,T.IPERIOD,T.DIM_DEPT,T.DIM_DEPTNAME,T.DIM_CUST,T.DIM_CUSTNAME,T.FEE_DC,T.FEE_D,T.FEE_Z,T.NMNY_DC_M,T.NMNY_D_M,T.NMNY_Z_M,T.NMNY_DC_Y,T.NMNY_D_Y,T.NMNY_Z_Y ,T.NCUSTCOUNT ,T.FEE_D_FT,T.FEE_Z_FT,
